...
class Train:
    ...
    def checkpoint(self):
        ...
        cp_path = self.checkpoint_save + '/checkpointdir'
        #print('self.checkpoint_save', self.checkpoint_save)
...
class train_pcd(Train):
    ...
    def Init(self,cvset):
        ...
        #print('datasets_defin:', self.args.datadir, "set{}".format(self.cvset), "train")
        ...
        self.dataset_train_loader = DataLoader(datasets.pcd(self.args.datadir + '/' + 'train'),
                                          num_workers=self.args.num_workers, batch_size=self.args.batch_size,
                                          shuffle=True)
        ...
        self.dataset_test = datasets.pcd(self.args.datadir + '/test')


        #self.checkpoint_save = pjoin(self.args.checkpointdir, folder_name, 'pcd', 'set{}'.format(self.cvset))
        self.checkpoint_save = self.args.checkpointdir + '/' + folder_name  #+ '/' + 'pcd' + 'set{}'.format(self.cvset)
...
if __name__ =="__main__":
    ...
    parser.add_argument('--datadir', default='./TSUNAMI')                                  #required=True)
    ...
    parser.add_argument('--epoch-save', type=int, default=100)
    parser.add_argument('--step-test', type=int, default=100)
    parser.add_argument('--encoder-arch', type=str,   default='resnet18')                  #required=True)
    ...
    if parser.parse_args().dataset == 'pcd':
        ...
        # Тут видимо подразумевается разбить весь сет на пять фолдов и тренировать по ним последовательно
        #for set in range(0, 5):
        #    train.Init(set)
        #    train.run()
        # Сделаем проще. Потренеруемся по всей выборке сразу.
        train.Init(0)
        train.run()

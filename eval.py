...
class evaluate_pcd(Evaluate):
    ...
    def run(self, set):
        ...
        #f_metrics = open(pjoin(self.resultdir, "eval_metrics(dataset).csv"), 'a+')
        f_metrics = open(self.resultdir + "/eval_metrics(dataset).csv", 'a+')

        metrics_writer = csv.writer(f_metrics)

        for ds in tqdm(['test']):
            #test_loader = datasets.pcd_eval(pjoin(self.args.datadir,ds))
            #print('self.args.datadir: -------->>>>    ', self.args.datadir, 'ds:', ds)
            test_loader = datasets.pcd_eval(self.args.datadir + '/' + ds)
...
if __name__ =='__main__':
    ...
    parser.add_argument('--resultdir' , default='./test_results')                               #required=True)
